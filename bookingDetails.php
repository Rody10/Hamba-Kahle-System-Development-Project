...
<header>
        ...
        <div class="cart">
            <button type="button" class="btn"><a href="maintainBooking.php" style="text-decoration: none; color: black;">BACK</a></button>
        </div>
    </header>
...
<h1 style="color: rgb(240, 127, 35); padding-left: 200px; padding-top: 50px; font-size: 45px;">Details</h1>
    ...
    $query = "SELECT bookingNumber, departure, destination, startDate, endDate, numberOfPassengers, vehicleType,
             firstName, lastName, contactNumber, emailAddress
             FROM booking
             INNER JOIN client ON client.clientId = booking.clientId where bookingNumber = $id;"; 
    ...
    while($row = mysqli_fetch_array($result)){
        ...
        echo "<h3>" . $row['firstName'] . " " . $row['lastName'] . "</h3>" . "<br>";
        echo "<strong>Departure: </strong>" . $row['departure'] . "<br>"; 
        ...
        echo "<strong>Passengers: </strong>" . $row['numberOfPassengers'] . "<br>"; 
        echo "<strong>Vehicle Type: </strong>" . $row['vehicleType'] . "<br>";
        echo "<strong>Client Number: </strong>" . $row['contactNumber'] . "<br>"; 
        echo "<strong>Client Email: </strong>" . $row['emailAddress'] . "<br>"; 
    }
    echo "</div>";
    mysqli_close($conn);
